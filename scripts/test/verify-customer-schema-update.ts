/**
 * Script to verify the customer schema update
 * 
 * This script:
 * 1. Connects to MongoDB
 * 2. Checks if the customer schema has been updated (no totalSpent, orderCount, lastOrderDate)
 * 3. Verifies that orders can still be retrieved for customers
 */
...
async function main() {
  try {
    ...
    console.log('\nVerification complete. The customer schema has been updated to remove redundant order-related fields.');
    console.log('The customer details dialog now calculates these values from actual orders in real-time.');
  } catch (error) {
